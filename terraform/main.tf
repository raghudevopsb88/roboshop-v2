...
module "ec2" {
  ...
  vpc_cidr_blocks   = module.vpc[local.network_key].all_vpc_cidr_blocks
  ...
  ssh_cidr_blocks   = concat(var.ssh_cidr_blocks, module.vpc[local.network_key].all_vpc_cidr_blocks)
  tags              = var.tags
}
...
module "eks" {
  ...
  vpc_id              = module.vpc[local.network_key].vpc_id
  subnets             = module.vpc[local.network_key].eks_subnet_ids
  cluster_subnets     = concat(module.vpc[local.network_key].app_subnet_ids, module.vpc[local.network_key].eks_subnet_ids)
  default_vpc_cidr    = data.aws_vpc.default.cidr_block
